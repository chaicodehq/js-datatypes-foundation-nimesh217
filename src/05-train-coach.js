...
export function findPassenger(passengers, name) {
  if (!Array.isArray(passengers) || typeof name != 'string')
    return undefined
  return passengers.find(p => { return p.name.toLowerCase() == name.toLowerCase() });
}

export function getPassengerIndex(passengers, name) {
  if (!Array.isArray(passengers) || typeof name != 'string')
    return -1
  return passengers.findIndex(p => { return p.name.toLowerCase() == name.toLowerCase() });
}

export function isAnyWaitlisted(passengers) {
  if (!Array.isArray(passengers))
    return false
  return passengers.some(p => { return p.status.toLowerCase() == 'waitlisted' });
}

export function areAllConfirmed(passengers) {
  if (!Array.isArray(passengers)|| passengers.length==0)
    return false
  return passengers.every(p => { return p.status.toLowerCase() == 'confirmed' });
}

export function getWaitlistedPassengers(passengers) {
   if (!Array.isArray(passengers))
    return []
  return passengers.filter(p => { return p.status.toLowerCase() == 'waitlisted' });
}
